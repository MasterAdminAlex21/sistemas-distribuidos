...
int SocketMulticast::recibe(PaqueteDatagrama & p, char* ipE){
	...
	multicast.imr_interface.s_addr = htonl( INADDR_ANY ); 
   	int opc = setsockopt( s, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&multicast, sizeof(multicast) );
   	if( opc < 0){
   		perror("error");
...
int SocketMulticast::recibeTimeout(PaqueteDatagrama & p, char* ipE, time_t segundos, suseconds_t microsegundos){
	ip_mreq multicast;
	multicast.imr_multiaddr.s_addr = inet_addr( ipE );
	multicast.imr_interface.s_addr = htonl( INADDR_ANY ); 
   	int opc = setsockopt( s, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&multicast, sizeof(multicast) );
